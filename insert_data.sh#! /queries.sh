...
echo "$($PSQL "SELECT DISTINCT(teams.name) FROM teams, games WHERE (teams.team_id = games.winner_id OR teams.team_id = games.opponent_id) AND (games.year = 2014 AND games.round = 'Eighth-Final') ORDER BY teams.name;")"
